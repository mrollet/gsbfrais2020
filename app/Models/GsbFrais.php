...
public function getInfosVisiteur($login, $mdp){
		...
		$visiteur = DB::table('visiteur')->where('login', '=', $login)
									   ->Where('mdp', '=', $mdp)
									   ->first();
		return $visiteur;
}
...
//            }
		$lesLignes = DB::table('lignefraishorsforfait')
...
*/
	public function getLesFraisForfait($idVisiteur, $mois){
        ...
        // $lesLignes = DB::select($req, ['idVisiteur'=>$idVisiteur, 'mois'=>$mois]);
		$lesLignes = DB::table('lignefraisforfait')
					->join('fraisforfait','lignefraisforfait.idfraisforfait','=','fraisforfait.id')
					->where('idvisiteur','=',$idVisiteur)
					->where('mois','=',$mois)
					->select('idVisiteur','mois','idfraisforfait','quantite','libelle','montant')
...
*/
	public function getLesIdFrais(){
		...
		$lesLignes = DB::table('fraisforfait')->orderBy('id')->get();
		return $lesLignes;
	}
/**
...
*/
	public function estPremierFraisMois($idVisiteur,$mois)
	{
		...
		$nb = DB::table('fichefrais')->where('mois','=',$mois)
									  ->where('idvisiteur','=',$idVisiteur)
									  ->count();
...
*/
	public function dernierMoisSaisi($idVisiteur){
		...
		$dernierMois = DB::table('fichefrais')->where('idvisiteur','=',$idVisiteur)->max('mois');
		return $dernierMois;
	}
...
*/
	public function creeNouvellesLignesFrais($idVisiteur,$mois){
		...
		DB::table('fichefrais')->insert(['idvisiteur' => $idVisiteur, 'mois' => $mois, 'nbjustificatifs' => 0, 'montantvalide' => 0, 'dateModif' => now(), 'idEtat' => 'CR']);
        //ne marche pas, pq????
        $lesIdFrais = $this->getLesIdFrais();
        //var_dump($lesIdFrais);
		foreach($lesIdFrais as $uneLigneIdFrais){
            Debug($lesIdFrais);
            $unIdFrais = $uneLigneIdFrais->id;
...
*/
	public function getUnFraisHorsForfait($idFrais){
        ...
        // $unFraisHF = $fraisHF[0];
        $unFraisHF = DB::table('lignefraishorsforfait')->where('id','=',$idFrais)->first();
        return $unFraisHF;
	}
/**
...
*/
	public function getLesFrais($idVisiteur, $mois){
        ...
        //         $lesLignes = DB::select($req, ['idVisiteur'=>$idVisiteur, 'mois'=>$mois]);
		$lesLignes = DB::table('fichefrais')->where('idvisiteur', '=',$idVisiteur)
										-> where('mois', '=',$mois)
										->get();
				return $lesLignes;
	}
...
*/
	public function getLesInfosFicheFrais($idVisiteur,$mois){
		...
		// $lesLignes = DB::select($req, ['idVisiteur'=>$idVisiteur,'mois'=>$mois]);
		// return $lesLignes[0];
		$laLigne = DB::table('fichefrais')->join('etat','id','=','idEtat')
										->where('idvisiteur', '=',$idVisiteur)
										-> where('mois', '=',$mois)
										->first();
		return $laLigne;
 ...
 */
	public function majEtatFicheFrais($idVisiteur,$mois,$etat){
		...
		DB::table('fichefrais')->where('idvisiteur','=',$idVisiteur)
								->where('mois','=',$mois)
								->update(['idetat'=>$etat, 'datemodif'=>now()]);
	}
}
?>
